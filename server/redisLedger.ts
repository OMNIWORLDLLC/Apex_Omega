...
export interface RouteGuardStep {
  venue: string;
  tokenIn: string;
  tokenOut: string;
}
...
function isLiveExecutionArmed() {
  if (runtimeLiveExecutionArmed !== null) return runtimeLiveExecutionArmed;
  return process.env.LIVE_EXECUTION === "true" &&
    process.env.SHADOW_MODE === "false" &&
    process.env.EXECUTION_DISABLED !== "true" &&
    process.env.DISCOVERY_ONLY_MODE !== "true";
}

export function setRedisLiveExecutionArmed(armed: boolean) {
  runtimeLiveExecutionArmed = armed;
}

function redisRequiredForThisRuntime() {
  return REDIS_REQUIRED_FOR_LIVE && isLiveExecutionArmed();
}
...
export function routeKeyFromC1Payload(flashloanAsset: string, steps: RouteGuardStep[]): string {
  const asset = String(flashloanAsset || "").toLowerCase();
  const stepParts = steps.map((step) =>
    `${String(step.venue ?? "").toLowerCase()}:${String(step.tokenIn ?? "").toLowerCase()}:${String(step.tokenOut ?? "").toLowerCase()}`
  );
  return `arb:lock:${asset}:${stepParts.join(":")}`;
}

export function routeKeyFromArbitrageRoute(route: ArbitrageRouteSummary): string {
  const parts = route.legs.map((leg) =>
    `${String(leg.venue ?? "").toLowerCase()}:${String(leg.tokenIn ?? "").toLowerCase()}:${String(leg.tokenOut ?? "").toLowerCase()}`
  );
  return `arb:lock:${String(route.tokenIn || "").toLowerCase()}:${String(route.tokenOut || "").toLowerCase()}:${parts.join(":")}`;
}

export function routeKeyFromC1InternalId(c1InternalId: string): string {
  return `arb:c2:lock:${String(c1InternalId || "").toLowerCase()}`;
}
...
export function routeKeyFromLiquidation(
  ...
  return [
    "arb:liq:lock",
    String(targetContract || "").toLowerCase(),
    String(user || "").toLowerCase(),
    String(debtAsset || "").toLowerCase(),
    String(collateralAsset || "").toLowerCase(),
  ].join(":");
}
...
export function opportunityId(payload: LedgerPayload) {
  if (payload.routeGuardKey) return hashPayload(String(payload.routeGuardKey));
...
export function getRedisLedgerStatus() {
  return {
    ...
    requiredForLive: REDIS_REQUIRED_FOR_LIVE,
    requiredNow: redisRequiredForThisRuntime(),
...
export async function getActiveLedgerOpportunities(limit = 100) {
  ...
  return rows.sort((left, right) => {
    const leftExecutable = left.status === "EXECUTABLE_PROFIT_CANDIDATE" || left.c1ExecutionEligible || left.executionReady;
    const rightExecutable = right.status === "EXECUTABLE_PROFIT_CANDIDATE" || right.c1ExecutionEligible || right.executionReady;
    if (leftExecutable !== rightExecutable) return rightExecutable ? 1 : -1;
    const profitDelta = Number(right.profit_usd ?? right.netProfitUsd ?? Number.NEGATIVE_INFINITY) -
...
export async function getLedgerOpportunityById(id: string) {
  const client = await getRedisClient();
  if (!client) return null;
  const raw = await client.hGet(opportunityKey(id), "payload");
...
export async function lockOpportunityForExecution(payload: LedgerPayload, ttlMs = EXECUTION_LOCK_TTL_MS) {
  ...
  const id = opportunityId(payload);
  if (!client) {
    if (redisRequiredForThisRuntime()) {
      return { ok: false, redis: false, id, reason: "REDIS_REQUIRED_FOR_LIVE_UNAVAILABLE" };
    }
    return { ok: true, redis: false, id, reason: "REDIS_UNAVAILABLE_MEMORY_LOCK_ONLY" };
  }
